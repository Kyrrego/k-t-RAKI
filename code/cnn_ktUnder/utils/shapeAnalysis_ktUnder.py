...
def extractDatCNN_ktUnder(acs, R, num_hid_layer, layer_design):
    '''
    ...
    rep_t_acs = (num_t_acs - Nk_t) // R
    rep_p_acs = (num_p_acs - Nk_p) // R
        ...
        for r in range(rep_r_acs):
            ...
            for t in range(rep_t_acs):
                ...
                trg_data[p, r, t, 0:acs.shape[0]] = acs[:, p*R +
                                                        (R-1), r+r_trg, t*R].reshape(acs.shape[0])    # top right point
                trg_data[p, r, t, acs.shape[0]:2*acs.shape[0]] = acs[:, p*R, r +
                                                                     r_trg, t*R+(R-1)].reshape(acs.shape[0])    # bottom left point
...
def fillDatCNN_ktUnder(kspace_zf, pred_mat, R, num_hid_layer, layer_design):
    '''
    ...
    rep_t = (num_t_data - Nk_t) // R
    rep_p = (num_p_data - Nk_p) // R
    print("The repetitions are (for data blocks): ",
    ...
    for p in range(rep_p):
        for r in range(rep_r):
            for t in range(rep_t):
                kspace_zf[:, p*R+(R-1), r+r_trg, t*R] = pred_mat[p, r,
                                                                 t, 0:num_coil].reshape((num_coil,))   # top right point
                # bottom left point
                kspace_zf[:, p*R, r+r_trg, t*R +
                          (R-1)] = pred_mat[p, r, t, num_coil:2*num_coil].reshape((num_coil,))
